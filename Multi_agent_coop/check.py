...
def Set_Prob_Robust(input, len_stl, cost):
    ...
    model_robust.setParam("MIPFocus", 3)
    zr = model_robust.addVars(len_stl + 1, 6, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="z_robust")
    uf2 = model_robust.addVars(len_stl, 2, lb=-u2max, ub=u2max, vtype=GRB.CONTINUOUS, name="w_robust")
    uf3 = model_robust.addVars(len_stl, 2, lb=-u3max, ub=u3max, vtype=GRB.CONTINUOUS, name="w_robust")
    add_model_constrs_robust(model_robust, input, zr, uf2, uf3, len_stl)
    obj = Add_obj_robust(model_robust, zr, cost, input, len_stl)
    model_robust.optimize()
    if (model_robust.status == GRB.Status.OPTIMAL):
        f2 = model_robust.getAttr('x', uf2)
        f3 = model_robust.getAttr('x', uf3)
...
def add_model_constrs_robust(model_robust, input, zr, uf2, uf3, len_stl):
    ...
    model_robust.addConstr((zr[0, 4] == optimal_state_sequence[0][4]), 'Known_state05')
    model_robust.addConstr((zr[0, 5] == optimal_state_sequence[0][5]), 'Known_state06')
    # system model_robust
    model_robust.addConstrs((zr[i + 1, 0] == zr[i, 0] + input[i][0] for i in range(len_stl)), 'Dynamic1')
    model_robust.addConstrs((zr[i + 1, 1] == zr[i, 1] + input[i][1] for i in range(len_stl)), 'Dynamic2')
    model_robust.addConstrs((zr[i + 1, 2] == zr[i, 2] + uf2[i, 0] for i in range(len_stl)), 'Dynamic3')
    model_robust.addConstrs((zr[i + 1, 3] == zr[i, 3] + uf2[i, 1] for i in range(len_stl)), 'Dynamic4')
    model_robust.addConstrs((zr[i + 1, 4] == zr[i, 4] + uf3[i, 0] for i in range(len_stl)), 'Dynamic5')
    model_robust.addConstrs((zr[i + 1, 5] == zr[i, 5] + uf3[i, 1] for i in range(len_stl)), 'Dynamic6')
    ...
    model_robust.addConstrs((xmap[0] <= zr[i, 2] for i in range(len_stl + 1)), 'Workspace1')
    model_robust.addConstrs((xmap[1] >= zr[i, 2] for i in range(len_stl + 1)), 'Workspace2')
    model_robust.addConstrs((ymap[0] <= zr[i, 3] for i in range(len_stl + 1)), 'Workspace3')
    model_robust.addConstrs((ymap[1] >= zr[i, 3] for i in range(len_stl + 1)), 'Workspace4')
    model_robust.addConstrs((xmap[0] <= zr[i, 4] for i in range(len_stl + 1)), 'Workspace1')
    model_robust.addConstrs((xmap[1] >= zr[i, 4] for i in range(len_stl + 1)), 'Workspace2')
    model_robust.addConstrs((ymap[0] <= zr[i, 5] for i in range(len_stl + 1)), 'Workspace3')
    model_robust.addConstrs((ymap[1] >= zr[i, 5] for i in range(len_stl + 1)), 'Workspace4')
    # physical constraints - speed


def Add_obj_robust(model_robust, zr, cost, input, len_stl):
    temp_GD = model_robust.addVars(len_stl + 1, 2, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="temp")
    rho_GD_temp = model_robust.addVars(len_stl + 1, 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="rho_GD_temp")
    rho_GD = model_robust.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="rho_GD")
    for i in range(len_stl + 1):
        model_robust.addConstr((temp_GD[i, 0] == 1-(zr[i, 2] - zr[i,0])*(zr[i, 2] - zr[i,0]) - (zr[i, 3] - zr[i,1])*(zr[i, 3] - zr[i,1])), "temp1")
        model_robust.addConstr((temp_GD[i, 1] == 1-(zr[i, 4] - zr[i,0])*(zr[i, 4] - zr[i,0]) - (zr[i, 5] - zr[i,1])*(zr[i, 5] - zr[i,1])), "temp1")
    for i in range(len_stl + 1):
        model_robust.addGenConstrMin(rho_GD_temp[i, 0], [temp_GD[i, j] for j in range(2)], name="minconstr")
    model_robust.addGenConstrMin(rho_GD, [rho_GD_temp[i, 0] for i in range(len_stl + 1)], name="minconstr2")
    model_robust.addConstr(rho_GD >= 0, "D")
    model_robust.update()
    ...
    for i in range(G2_t + 1):
        model_robust.addConstr((temp_G2[i, 0] == zr[G2_tmin + i, 2] - xB2[0]), "temp1")
        model_robust.addConstr((temp_G2[i, 1] == xB2[1] - zr[G2_tmin + i, 2]), "temp1")
        model_robust.addConstr((temp_G2[i, 2] == zr[G2_tmin + i, 3] - yB2[0]), "temp1")
        model_robust.addConstr((temp_G2[i, 3] == yB2[1] - zr[G2_tmin + i, 3]), "temp1")
    for i in range(G2_t + 1):
    ...
    for i in range(F2_t + 1):
        model_robust.addConstr((temp_F2[i, 0] == zr[F2_tmin + i, 2] - xB1[0]), "temp1")
        model_robust.addConstr((temp_F2[i, 1] == xB1[1] - zr[F2_tmin + i, 2]), "temp1")
        model_robust.addConstr((temp_F2[i, 2] == zr[F2_tmin + i, 3] - yB1[0]), "temp1")
        model_robust.addConstr((temp_F2[i, 3] == yB1[1] - zr[F2_tmin + i, 3]), "temp1")
    for i in range(F2_t + 1):
    ...
    temp_F3 = model_robust.addVars(F3_t + 1, 4, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="temp")
    rho_F3_temp = model_robust.addVars(F3_t + 1, 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="rho_F3_temp")
    rho_F3 = model_robust.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="rho_F3")
    for i in range(F3_t + 1):
        model_robust.addConstr((temp_F3[i, 0] == zr[F3_tmin + i, 2] - xC1[0]), "temp1")
        model_robust.addConstr((temp_F3[i, 1] == xC1[1] - zr[F3_tmin + i, 2]), "temp1")
        model_robust.addConstr((temp_F3[i, 2] == zr[F3_tmin + i, 3] - yC1[0]), "temp1")
        model_robust.addConstr((temp_F3[i, 3] == yC1[1] - zr[F3_tmin + i, 3]), "temp1")
    for i in range(F3_t + 1):
        model_robust.addGenConstrMin(rho_F3_temp[i, 0], [temp_F3[i, j] for j in range(4)], name="minconstr1")
    model_robust.addGenConstrMax(rho_F3, [rho_F3_temp[i, 0] for i in range(F3_t + 1)], name="minconstr2")

    rho_F = model_robust.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="rho")

    model_robust.addGenConstrMin(rho_F,  [rho_F2, rho_G2, rho_F3], name="minconstr")
    ...
    model_robust.addGenConstrMin(obj, [rho_FL, rho_FK], name="minconstr")
