...
def Set_Prob(uf_set, len_stl):
    ...
    # add variants
    z = model.addVars(len_uf_set * (len_stl + 1), 6, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS,
                      name="z")  # system state
...
def add_model_constrs_with_uf(model, z, u, uf_set, len_uf_set, len_stl):
    for j in range(len_uf_set):
        ...
        model.addConstr((z[j * (len_stl + 1), 4] == optimal_state_sequence[0][4]), 'Known_state05')
        model.addConstr((z[j * (len_stl + 1), 5] == optimal_state_sequence[0][5]), 'Known_state06')

    # system model
    for j in range(len_uf_set):
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 0] == z[i + j * (len_stl + 1), 0] + u[i, 0] for i in range(len_stl)),
                         'Dynamic_x1')
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 1] == z[i + j * (len_stl + 1), 1] + u[i, 1] for i in range(len_stl)),
                         'Dynamic_y1')
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 2] == z[i + j * (len_stl + 1), 2] + uf_set[j][i][0] for i in range(len_stl)), 'Dynamic_x2')
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 3] == z[i + j * (len_stl + 1), 3] + uf_set[j][i][1] for i in range(len_stl)), 'Dynamic_y2')
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 4] == z[i + j * (len_stl + 1), 4] + uf_set[j][i][2] for i in range(len_stl)), 'Dynamic_x3')
        model.addConstrs((z[i + 1 + j * (len_stl + 1), 5] == z[i + j * (len_stl + 1), 5] + uf_set[j][i][3] for i in range(len_stl)), 'Dynamic_y3')
        ...
        model.addConstrs((xmap[0] <= z[i + j * (len_stl + 1), 2] for i in range(1, len_stl + 1)), 'Workspace1')
        model.addConstrs((xmap[1] >= z[i + j * (len_stl + 1), 2] for i in range(1, len_stl + 1)), 'Workspace2')
        model.addConstrs((ymap[0] <= z[i + j * (len_stl + 1), 3] for i in range(1, len_stl + 1)), 'Workspace3')
        model.addConstrs((ymap[1] >= z[i + j * (len_stl + 1), 3] for i in range(1, len_stl + 1)), 'Workspace4')
        model.addConstrs((xmap[0] <= z[i + j * (len_stl + 1), 4] for i in range(1, len_stl + 1)), 'Workspace1')
        model.addConstrs((xmap[1] >= z[i + j * (len_stl + 1), 4] for i in range(1, len_stl + 1)), 'Workspace2')
        model.addConstrs((ymap[0] <= z[i + j * (len_stl + 1), 5] for i in range(1, len_stl + 1)), 'Workspace3')
        model.addConstrs((ymap[1] >= z[i + j * (len_stl + 1), 5] for i in range(1, len_stl + 1)), 'Workspace4')
        model.update()
...
def add_stl_constrs_binary(model, z, len_uf_set, len_stl):
    ...
    F3 = model.addVars(len_uf_set * (F3_t + 1), 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="F0C")
    F3_4 = model.addVars(len_uf_set * (F3_t + 1), 4, vtype=GRB.BINARY, name="F0_4")

    GD = model.addVars(len_uf_set * (len_stl + 1), 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="G01")
    GD_2 = model.addVars(len_uf_set * (len_stl + 1), 2, vtype=GRB.BINARY, name="G02")
...
# consider F2 and G2, exist uf such that phi f is satisfied
    ...
    F3_max = model.addVars(len_uf_set, 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="F0C")

    G2_min = model.addVars(len_uf_set, 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="F0C")

    GD_min = model.addVars(len_uf_set, 1, lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name="F0C")
    ...
    for j in range(len_uf_set):
        ...
        model.addGenConstrMax(F3_max[j, 0], [F3[i + j * (F3_t + 1), 0] for i in range(F3_t + 1)], name="maxconstr")
        model.addGenConstrMin(G2_min[j, 0], [G2[i + j * (G2_t + 1), 0] for i in range(G2_t + 1)], name="minconstr")
        model.addGenConstrMin(GD_min[j, 0], [GD[i + j * (len_stl + 1), 0] for i in range(len_stl + 1)], name="minconstr")
        model.addGenConstrMin(F[j, 0], [F2_max[j, 0], F3_max[j, 0], G2_min[j, 0], GD_min[j, 0]], name="minconstr")
...
# consider phi f implys phi l for all uf
    ...
    for j in range(len_uf_set):
        ...
        model.addConstrs((z[j * (len_stl + 1) + F2_tmin + i, 2] - xB1[0] <= M * F2_4[i + j * (F2_t + 1), 0] for i in range(F2_t + 1)), 'F4')
        model.addConstrs((-z[j * (len_stl + 1) + F2_tmin + i, 2] + xB1[0] <= M * (1 - F2_4[i + j * (F2_t + 1), 0]) for i in range(F2_t + 1)), 'F5')
        model.addConstrs((-z[j * (len_stl + 1) + F2_tmin + i, 2] + xB1[1] <= M * F2_4[i + j * (F2_t + 1), 1] for i in range(F2_t + 1)), 'F6')
        model.addConstrs((z[j * (len_stl + 1) + F2_tmin + i, 2] - xB1[1] <= M * (1 - F2_4[i + j * (F2_t + 1), 1]) for i in range(F2_t + 1)), 'F7')
        model.addConstrs((z[j * (len_stl + 1) + F2_tmin + i, 3] - yB1[0] <= M * F2_4[i + j * (F2_t + 1), 2] for i in range(F2_t + 1)), 'F8')
        model.addConstrs((-z[j * (len_stl + 1) + F2_tmin + i, 3] + yB1[0] <= M * (1 - F2_4[i + j * (F2_t + 1), 2]) for i in range(F2_t + 1)), 'F9')
        model.addConstrs((-z[j * (len_stl + 1) + F2_tmin + i, 3] + yB1[1] <= M * F2_4[i + j * (F2_t + 1), 3] for i in range(F2_t + 1)), 'F00')
        model.addConstrs((z[j * (len_stl + 1) + F2_tmin + i, 3] - yB1[1] <= M * (1 - F2_4[i + j * (F2_t + 1), 3]) for i in range(F2_t + 1)), 'F01')
        for i in range(F3_t + 1):
            model.addConstrs((F3[i + j * (F3_t + 1), 0] <= F3_4[i + j * (F3_t + 1), p] for p in range(4)), 'F2')
            model.addConstr((sum(F3_4[i + j * (F3_t + 1), p] for p in range(4)) - 3 <= F3[i + j * (F3_t + 1), 0]), 'F3')
        model.addConstrs((z[j * (len_stl + 1) + F3_tmin + i, 4] - xC1[0] <= M * F3_4[i + j * (F3_t + 1), 0] for i in range(F3_t + 1)), 'F4')
        model.addConstrs((-z[j * (len_stl + 1) + F3_tmin + i, 4] + xC1[0] <= M * (1 - F3_4[i + j * (F3_t + 1), 0]) for i in range(F3_t + 1)), 'F5')
        model.addConstrs((-z[j * (len_stl + 1) + F3_tmin + i, 4] + xC1[1] <= M * F3_4[i + j * (F3_t + 1), 1] for i in range(F3_t + 1)), 'F6')
        model.addConstrs((z[j * (len_stl + 1) + F3_tmin + i, 4] - xC1[1] <= M * (1 - F3_4[i + j * (F3_t + 1), 1]) for i in range(F3_t + 1)), 'F7')
        model.addConstrs((z[j * (len_stl + 1) + F3_tmin + i, 5] - yC1[0] <= M * F3_4[i + j * (F3_t + 1), 2] for i in range(F3_t + 1)), 'F8')
        model.addConstrs((-z[j * (len_stl + 1) + F3_tmin + i, 5] + yC1[0] <= M * (1 - F3_4[i + j * (F3_t + 1), 2]) for i in range(F3_t + 1)), 'F9')
        model.addConstrs((-z[j * (len_stl + 1) + F3_tmin + i, 5] + yC1[1] <= M * F3_4[i + j * (F3_t + 1), 3] for i in range(F3_t + 1)), 'F00')
        model.addConstrs((z[j * (len_stl + 1) + F3_tmin + i, 5] - yC1[1] <= M * (1 - F3_4[i + j * (F3_t + 1), 3]) for i in range(F3_t + 1)), 'F01')
        ...
        model.addConstrs((z[j * (len_stl + 1) + G2_tmin + i, 2] - xB2[0] <= M * G2_4[i + j * (G2_t + 1), 0] for i in range(G2_t + 1)), 'G06')
        model.addConstrs((-z[j * (len_stl + 1) + G2_tmin + i, 2] + xB2[0] <= M * (1 - G2_4[i + j * (G2_t + 1), 0]) for i in range(G2_t + 1)), 'G07')
        model.addConstrs((-z[j * (len_stl + 1) + G2_tmin + i, 2] + xB2[1] <= M * G2_4[i + j * (G2_t + 1), 1] for i in range(G2_t + 1)), 'G08')
        model.addConstrs((z[j * (len_stl + 1) + G2_tmin + i, 2] - xB2[1] <= M * (1 - G2_4[i + j * (G2_t + 1), 1]) for i in range(G2_t + 1)), 'G09')
        model.addConstrs((z[j * (len_stl + 1) + G2_tmin + i, 3] - yB2[0] <= M * G2_4[i + j * (G2_t + 1), 2] for i in range(G2_t + 1)), 'G010')
        model.addConstrs((-z[j * (len_stl + 1) + G2_tmin + i, 3] + yB2[0] <= M * (1 - G2_4[i + j * (G2_t + 1), 2]) for i in range(G2_t + 1)), 'G011')
        model.addConstrs((-z[j * (len_stl + 1) + G2_tmin + i, 3] + yB2[1] <= M * G2_4[i + j * (G2_t + 1), 3] for i in range(G2_t + 1)), 'G012')
        model.addConstrs((z[j * (len_stl + 1) + G2_tmin + i, 3] - yB2[1] <= M * (1 - G2_4[i + j * (G2_t + 1), 3]) for i in range(G2_t + 1)), 'G013')
        for i in range(len_stl + 1):
            model.addConstrs((GD[i + j * (len_stl + 1), 0] <= GD_2[i + j * (len_stl + 1), p] for p in range(2)), 'G04')
            model.addConstr((sum(GD_2[i + j * (len_stl + 1), p] for p in range(2)) - 1 <= GD[i + j * (len_stl + 1), 0]), 'G05')
        model.addConstrs((1-(z[j * (len_stl + 1) + i, 2] - z[j * (len_stl + 1) + i, 0])*(z[j * (len_stl + 1) + i, 2] - z[j * (len_stl + 1) + i, 0]) - (z[j * (len_stl + 1) + i, 3] - z[j * (len_stl + 1) + i, 1])*(z[j * (len_stl + 1) + i, 3] - z[j * (len_stl + 1) + i, 1]) <= M * GD_2[i + j * (len_stl + 1), 0] for i in range(len_stl + 1)), 'G06')
        model.addConstrs(((z[j * (len_stl + 1) + i, 2] - z[j * (len_stl + 1) + i, 0])*(z[j * (len_stl + 1) + i, 2] - z[j * (len_stl + 1) + i, 0]) + (z[j * (len_stl + 1) + i, 3] - z[j * (len_stl + 1) + i, 1])*(z[j * (len_stl + 1) + i, 3] - z[j * (len_stl + 1) + i, 1]) - 1 <= M * (1 - GD_2[i + j * (len_stl + 1), 0]) for i in range(len_stl + 1)), 'G06')
        model.addConstrs((1-(z[j * (len_stl + 1) + i, 4] - z[j * (len_stl + 1) + i, 0])*(z[j * (len_stl + 1) + i, 4] - z[j * (len_stl + 1) + i, 0]) - (z[j * (len_stl + 1) + i, 5] - z[j * (len_stl + 1) + i, 1])*(z[j * (len_stl + 1) + i, 5] - z[j * (len_stl + 1) + i, 1]) <= M * GD_2[i + j * (len_stl + 1), 1] for i in range(len_stl + 1)), 'G06')
        model.addConstrs(((z[j * (len_stl + 1) + i, 4] - z[j * (len_stl + 1) + i, 0])*(z[j * (len_stl + 1) + i, 4] - z[j * (len_stl + 1) + i, 0]) + (z[j * (len_stl + 1) + i, 5] - z[j * (len_stl + 1) + i, 1])*(z[j * (len_stl + 1) + i, 5] - z[j * (len_stl + 1) + i, 1]) - 1 <= M * (1 - GD_2[i + j * (len_stl + 1), 1]) for i in range(len_stl + 1)), 'G06')
